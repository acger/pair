...
func (c *Client) readPump() {
	...
	for {
		...
		chatMsg := model.ClientChatMsg{}
		jsoniter.Unmarshal(message, &chatMsg)
		chatMsg.ClientNum = len(c.hub.clients)
		broadcastMsg, _ := jsoniter.Marshal(chatMsg)
...
func (c *Client) sendToQueue(message []byte) {
	c.hub.manager.svcCtx.KqPusher.Push(string(message))
}
...
func ServeWs(manager *Manager, w http.ResponseWriter, r *http.Request) {
	...
	hub := manager.NewHub(roomName, uid, toUid)
